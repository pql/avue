...
export const list = ({
  ...
  return new Promise((resolve) => {
    ...
    let count_sql, count_key = 'count(*) as count'
    if (sql.indexOf('*') != -1) {
      count_sql = sql.replace('*', count_key);
    } else {
      count_sql = 'SELECT ' + count_key + sql.substr(sql.indexOf(' FROM'))
    }
